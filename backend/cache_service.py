"""
Cache Service - Redis caching pour optimisation performances
"""
...
# Cache in-memory fallback si Redis non disponible
_memory_cache: dict = {}
...
async def invalidate_user_cache(user_id: str):
    ...
    await cache.clear_pattern(f"*:{user_id}:*")
...
async def invalidate_analytics_cache(user_id: str = None):
    ...
    if user_id:
        await cache.clear_pattern(f"analytics:*:{user_id}:*")
    else:
