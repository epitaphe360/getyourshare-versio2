...
async def request_payout(current_user: dict = Depends(get_current_user_from_cookie)):
...
@app.get("/api/affiliate-links")
async def get_affiliate_links_endpoint(current_user: dict = Depends(get_current_user_from_cookie)):
...
async def get_gamification_status(
    ...
    try:
        ...
        if payload["sub"] != user_id and current_user.get("role") != "admin":
