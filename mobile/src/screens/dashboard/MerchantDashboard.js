...
const MerchantDashboard = ({navigation}) => {
  ...
  return (
    <ScrollView
      ...
      <Text style={styles.period}>Performance du mois</Text>

      <View style={styles.statsGrid}>
        <StatCard label="Produits actifs" value={stats?.active_products} color="#3b82f6" />
        <StatCard label="Influenceurs" value={stats?.active_influencers} color="#8b5cf6" />
        <StatCard label="Conversions" value={stats?.total_conversions} color="#10b981" />
        <StatCard label="CA (MAD)" value={stats?.total_revenue} color="#f59e0b" />
      </View>

      <TouchableOpacity
        style={styles.cta}
        onPress={() => navigation.navigate('Products')}>
        <Text style={styles.ctaText}>Gérer mes produits →</Text>
      </TouchableOpacity>

      <TouchableOpacity
        style={[styles.cta, {backgroundColor: '#8b5cf6'}]}
        onPress={() => navigation.navigate('Affiliations')}>
        <Text style={styles.ctaText}>Demandes d'affiliation →</Text>
      </TouchableOpacity>
    </ScrollView>
  );
};
...
const styles = StyleSheet.create({
  ...
  cta: {backgroundColor: '#3b82f6', borderRadius: 12, padding: 16, alignItems: 'center', marginBottom: 12},
